...
Bias-aware Streamlit app for field-survey planning from GBIF occurrence CSV files
or direct scientific-name searches via the GBIF API.
"""
...
MEDIA_CANDIDATES = ["mediaurl", "media_url", "imageurl", "image_url", "identifier", "associatedmedia", "associated_media", "写真", "画像"]
GBIF_ID_CANDIDATES = ["gbifid", "gbif_id", "key", "occurrenceid", "occurrence_id"]
LOCALITY_CANDIDATES = ["locality", "municipality", "county", "stateprovince", "location", "場所", "地点"]
...
def detect_columns(df: pd.DataFrame) -> ColumnDetection:
    ...
    if lat is None or lon is None:
        raise ValueError("Latitude/longitude columns could not be detected. Use decimalLatitude/decimalLongitude or latitude/longitude.")
...
def fetch_gbif_occurrences(scientific_name: str, max_records: int, country_code: str = "", year_from: Optional[int] = None, year_to: Optional[int] = None) -> tuple[GBIFTaxonMatch, pd.DataFrame]:
...
def make_candidate_sites(df: pd.DataFrame, method: str, thinning_m: float) -> pd.DataFrame:
    columns = ["site_id", "cluster_id", "latitude", "longitude", "n_occurrences", "species_summary", "year_min", "year_max", "representative_gbif_id", "representative_media_url", "representative_locality", "candidate_method", "selection_reason", "bias_warning", "priority_score"]
    ...
    for site_id, (cluster_id, group) in enumerate(clustered.groupby("cluster_id", sort=True), start=1):
        ...
        sites.append({"site_id": site_id, "cluster_id": int(cluster_id), "latitude": lat, "longitude": lon, "n_occurrences": n, "species_summary": summarize_species(group.get("_species", pd.Series(dtype=str))), "year_min": year_min, "year_max": year_max, "representative_gbif_id": str(rep.get("_gbif_id", "")), "representative_media_url": str(rep.get("_media_url", "")), "representative_locality": str(rep.get("_locality", "")), "candidate_method": method, "selection_reason": reason, "bias_warning": warning, "priority_score": priority})
...
def add_priority_rank(sites: pd.DataFrame) -> pd.DataFrame:
    ...
    if out.empty:
        out["priority_rank"] = []
        return out
    priority_order = out.sort_values(["priority_score", "n_occurrences"], ascending=[False, False]).reset_index(drop=True)
    priority_order["priority_rank"] = range(1, len(priority_order) + 1)
    out = out.merge(priority_order[["site_id", "priority_rank"]], on="site_id", how="left")
    return out
...
def make_google_maps_transit_url(origin: tuple[float, float], destination: tuple[float, float]) -> str:
    params = {"api": "1", "origin": f"{origin[0]:.6f},{origin[1]:.6f}", "destination": f"{destination[0]:.6f},{destination[1]:.6f}", "travelmode": "transit"}
...
def build_public_transit_legs(sites: pd.DataFrame) -> pd.DataFrame:
    ...
    for i in range(len(ordered) - 1):
        ...
        rows.append({"leg": i + 1, "from_site": int(a["site_id"]), "to_site": int(b["site_id"]), "google_maps_transit_url": make_google_maps_transit_url(origin, dest), "fare_yen": 0, "notes": "Enter actual train/bus/ferry fare checked manually."})
    return pd.DataFrame(rows)


def transit_cost_summary(legs: pd.DataFrame, origin_roundtrip_yen: float, lodging_per_night: float, nights: int, per_diem: float, days: int, misc: float) -> dict[str, float]:
    ...
    return {"inter_site_public_transport_yen": round(float(fares)), "origin_roundtrip_yen": round(float(origin_roundtrip_yen)), "lodging_yen": round(float(lodging)), "per_diem_yen": round(float(daily)), "misc_yen": round(float(misc)), "estimated_total_yen": round(float(total))}
...
def build_map(occurrences: pd.DataFrame, sites: pd.DataFrame, buffer_radius_m: float, show_occurrences: bool, show_buffers: bool, show_clusters: bool, show_candidate_sites: bool, show_routes: bool, show_distance_labels: bool) -> folium.Map:
    ...
    if show_candidate_sites:
        ...
        for _, row in sites.iterrows():
            ...
            folium.Marker(location=(row["latitude"], row["longitude"]), popup=folium.Popup(popup_html_site(row), max_width=380), tooltip=f"Site {int(row['site_id'])} / Priority {row.get('priority_rank', '')}", icon=folium.DivIcon(html="<div style='font-size:22px;line-height:22px;color:red;text-shadow:0 0 2px white,0 0 4px white;'>★</div>")).add_to(group)
...
def load_input_dataframe_from_ui() -> tuple[Optional[pd.DataFrame], str]:
    ...
    with st.spinner("Fetching GBIF occurrences..."):
        match, df = fetch_gbif_occurrences(scientific_name, int(max_records), country_code, year_from, year_to)
    message = f"GBIF match: {match.matched_name or match.input_name} / usageKey={match.usage_key} / confidence={match.confidence}. Fetched {len(df):,} raw occurrence records."
    return df, message
...
def main() -> None:
    ...
    st.caption("Bias-aware field survey planning from GBIF occurrence data, with photo evidence and public-transit fare calculation.")
    ...
    col1.metric("Raw valid occurrences", f"{len(occ_raw):,}")
    ...
    priority_cols = ["priority_rank", "site_id", "route_order", "priority_score", "n_occurrences", "latitude", "longitude", "year_min", "year_max", "bias_warning", "selection_reason", "representative_media_url"]
    ...
    if ordered_sites.empty:
        ...
        display_cols = ["route_order", "priority_rank", "site_id", "cluster_id", "latitude", "longitude", "n_occurrences", "priority_score", "next_site_straight_km", "species_summary", "year_min", "year_max", "candidate_method", "bias_warning", "selection_reason", "representative_gbif_id", "representative_media_url", "google_maps_point_url"]
    ...
    with dl1:
        st.download_button("Download HTML map", html_bytes, "gbif_fieldmap.html", "text/html", width="stretch")
    with dl2:
