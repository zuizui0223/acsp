#!/usr/bin/env python3
"""Predeclare an untouched factorial cohort for robust-patch confirmation.

This script samples taxon-region identities only. It does not fetch focal
occurrences for selected taxa, build candidate surfaces, calculate robust
support, or inspect held-out coordinates/outcomes.
"""
...
PROTOCOL_PATH = Path("validation/acsp_robust_patch_untouched_confirmation_protocol.json")
EXPECTED_PROTOCOL = "1d35e5c49a800f63b7f29f55001163c5be3e2d2436dc49d22cbd81ce9f221818"
...
def _canonical_fingerprint(path: Path) -> tuple[dict[str, object], str]:
    ...
    if expected != calculated or calculated != EXPECTED_PROTOCOL:
        raise ValueError(
            "robust-patch confirmation protocol fingerprint mismatch: "
...
def run(protocol_path: Path, output: Path) -> dict[str, object]:
    ...
    if expected_pairs != expected_from_design:
        raise ValueError(
            f"factorial cohort dimensions imply {expected_from_design} pairs, "
            f"not declared {expected_pairs}"
        )
    ...
    exclusion_rows.extend(
        {
            "scientific_name": prefix,
            "exclusion_source": "protocol",
            "exclusion_type": "prefix",
        }
    ...
    for region_index, cell_tuple in enumerate(REGION_CELLS):
        ...
        for group in groups:
            ...
            for record_stratum in range(n_strata):
                ...
                child_index += 1
                stratum = eligible.loc[
                    eligible["record_count_stratum"].eq(record_stratum)
                ].copy()
                ...
                audits.append(
                    {
                        ...
                        "post_protocol_exclusion_frame": int(len(eligible)),
                ...
                if len(available) < per_cell:
                    raise RuntimeError(
                        f"insufficient untouched taxa for {region_name}/{group}/"
                        f"stratum{record_stratum}: available={len(available)}, required={per_cell}"
                    )
    ...
    if declared["scientific_name"].duplicated().any():
        raise RuntimeError("duplicate declared taxa")
    ...
    pd.DataFrame(exclusion_rows).drop_duplicates().sort_values(
        ["exclusion_type", "scientific_name", "exclusion_source"], kind="mergesort"
    ).to_csv(output / "excluded_taxa.csv", index=False)
    ...
    result = {
        ...
        "record_count_stratum_counts": {
            str(key): int(value) for key, value in record_counts.items()
        },
        "sampling_frame_cells": int(len(audit)),
        "minimum_available_unused_in_cell_stratum": int(
            audit["available_unused_before_draw"].min()
        ),
...
def parser() -> argparse.ArgumentParser:
    ...
    command.add_argument(
        "--output",
        type=Path,
        default=Path("robust-patch-untouched-confirmation-cohort"),
    )
    return command
