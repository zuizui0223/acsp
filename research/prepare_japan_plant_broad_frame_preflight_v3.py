#!/usr/bin/env python3
"""Historical-only qualification and candidate-frame freeze for issue #202.

Input is the exact 48 stage-1 identity candidates. This script may open only
JP 2000-2020 occurrence evidence and WorldCover candidate membership. It never
queries 2021-2025 outcomes or evaluates a selector.
"""
...
from acsp.discovery import attach_nearest_anchor_distance, build_rectangular_candidate_frame, cluster_medoid_table, complete_link_clusters
...
from prepare_cirsium_disjoint_broad_frame_replication_v1 import _hash_candidate_ids, _hash_rows, _strict_exact, _choose_training_region
...
def _protocol() -> dict[str, Any]:
    ...
    if int(stage["candidate_identity_count"]) != 48 or int(stage["final_taxa"]) != 24:
...
def _verify_stage1(candidates_path: Path, audit_path: Path) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    for key in ("focal_2000_2020_occurrences_opened", "heldout_2021_2025_opened", "candidate_frames_built", "selector_evaluated"):
...
def _build_universes(species_key: int, selected_historical: pd.DataFrame, region: dict[str, Any], cfg: dict[str, Any]) -> dict[str, Any]:
    universe = cfg["candidate_universes"]
    clusters = complete_link_clusters(selected_historical, radius_km=0.5)
    anchors = cluster_medoid_table(clusters, prefix=f"JPB3_{int(species_key)}_H")
    ...
    broad_raw, broad_audit = build_rectangular_candidate_frame(
        ...
        candidate_id_prefix=f"JPB3_{int(species_key)}_{region['region_id']}_broad",
    )
    broad_raw = attach_nearest_anchor_distance(broad_raw, anchors)
    broad_raw = broad_raw.loc[pd.to_numeric(broad_raw["nearest_anchor_km"], errors="coerce").ge(float(universe["known_exclusion_km"]) - 1e-12)].copy().reset_index(drop=True)
    ...
    for radius in universe["local_outer_radii_km"]:
        lanes[f"LOCAL_{int(radius)}KM_LAND"] = broad_land.loc[nearest.le(float(radius) + 1e-12)].copy().reset_index(drop=True)
    return {
        "historical_evidence_sha256": _hash_rows(selected_historical, ["occurrence_id", "latitude", "longitude", "event_year", "coordinate_uncertainty_m"]),
        "population_anchor_sha256": _hash_rows(anchors, ["occurrence_id", "latitude", "longitude", "cluster_size"]),
        "historical_population_count": int(len(anchors)),
        "selected_region": {"region_id": str(region["region_id"]), "region_name": str(region["region_name"]), "west": float(region["west"]), "south": float(region["south"]), "east": float(region["east"]), "north": float(region["north"])},
        "broad_frame_audit": broad_audit.__dict__,
        "worldcover_point_audit": wc_audit.as_dict(),
        "candidate_universes": {name: {"candidate_count": int(len(frame)), "candidate_id_sha256": _hash_candidate_ids(frame)} for name, frame in lanes.items()},
    }


def select_final_qualifiers(qualifier_rows: list[dict[str, Any]], *, final_count: int, seed: int) -> list[dict[str, Any]]:
    ...
    for row in qualifier_rows:
        ...
        work.append(item)
    return sorted(work, key=lambda x: (x["final_selection_hash"], int(x["speciesKey"])))[: int(final_count)]


def run(candidates_path: Path, stage1_audit_path: Path) -> dict[str, Any]:
    cfg = _protocol()
    stage = cfg["stage2_historical_qualification"]
    candidates, stage1_audit = _verify_stage1(candidates_path, stage1_audit_path)
    ...
    regions = _region_by_id()

    for row in candidates.itertuples(index=False):
        key = int(row.speciesKey)
        name = str(row.scientific_name)
        historical, provider_audit = fetch_gbif_occurrence_evidence(name, country="JP", year_from=2000, year_to=2020, maximum_records=10000)
        if int(provider_audit.matched_usage_key) != key:
            raise RuntimeError(f"provider identity drift for speciesKey={key}: {provider_audit.matched_usage_key}")
        strict = _strict_exact(historical, max_uncertainty)
        region, selected_historical, diagnostics = _choose_training_region(strict, allowed_region_ids=_region_ids(), cluster_radius_km=0.5)
        population_count = 0 if region is None else int(len(complete_link_clusters(selected_historical, radius_km=0.5)))
        ...
        ledger.append({
            ...
            "qualified": qualified,
            "qualification_state": "QUALIFIED" if qualified else ("NO_HISTORICAL_ANCHOR" if region is None else "BELOW_MIN_HISTORICAL_POPULATIONS"),
        })
        if qualified:
            q = {"candidate_id": int(row.candidate_id), "speciesKey": key, "scientific_name": name, "selected_region": str(region["region_id"]), "historical_population_count": population_count}
            qualifier_rows.append(q)
            state_cache[key] = (selected_historical, region)

    final_rows = select_final_qualifiers(qualifier_rows, final_count=int(stage["final_taxa"]), seed=int(stage["final_selection_seed"]))
    if not final_rows:
        return {
            "schema_version": "japan-plant-broad-frame-preflight-v3",
    ...
    canonical = [{"speciesKey": int(row["speciesKey"]), "scientific_name": str(row["scientific_name"]), "selected_region": str(row["selected_region"]["region_id"]), "historical_evidence_sha256": str(row["historical_evidence_sha256"]), "population_anchor_sha256": str(row["population_anchor_sha256"]), "candidate_universes": row["candidate_universes"]} for row in selected]
    return {
        "schema_version": "japan-plant-broad-frame-preflight-v3",
...
def main() -> int:
    ...
    print(json.dumps({k: v for k, v in result.items() if k not in {"qualification_ledger", "selected_taxa"}}, ensure_ascii=False, indent=2))
    return 0
