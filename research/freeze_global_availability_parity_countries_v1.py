#!/usr/bin/env python3
"""Stage 2 for issue #197: freeze historical-only country plans for 48 identities.

This stage must consume an externally pinned stage-1 identity artifact. It opens
...
Transient/technical historical provider errors abort the stage rather than being
reclassified as scientific PROVIDER_BLOCKED states.
"""
...
def verify_stage1(identities_path: Path, audit_path: Path, *, expected_identity_sha256: str, expected_audit_sha256: str) -> tuple[pd.DataFrame, dict[str, object]]:
    identities_path = Path(identities_path)
    audit_path = Path(audit_path)
    if _sha256_file(identities_path) != str(expected_identity_sha256):
        raise ValueError("stage1 identity artifact SHA256 mismatch")
    if _sha256_file(audit_path) != str(expected_audit_sha256):
        raise ValueError("stage1 audit artifact SHA256 mismatch")
    identities = pd.read_csv(identities_path)
    audit = json.loads(audit_path.read_text(encoding="utf-8"))
    if audit.get("status") != "IDENTITY_FREEZE_COMPLETE_PRE_HISTORICAL_COUNTRY_QUERY":
        raise ValueError("stage1 audit is not an identity-only complete freeze")
    if audit.get("focal_historical_country_facets_opened") is not False:
        raise ValueError("stage1 already opened focal historical countries")
    if audit.get("heldout_2021_2025_opened") is not False:
        raise ValueError("stage1 already opened heldout")
    ...
    if missing:
        raise ValueError(f"stage1 identities missing columns: {missing}")
    if len(identities) != 48 or identities["speciesKey"].nunique() != 48 or identities["scientific_name"].nunique() != 48:
        raise ValueError("stage1 identity artifact is not 48 unique taxa")
    if identities["taxon_group"].value_counts().to_dict() != {"plant": 24, "animal": 24}:
        raise ValueError("stage1 identity group balance drift")
...
def fetch_historical_counts_once(
    ...
    *,
    ...
    raise RuntimeError(f"historical provider failed after {attempts} attempts for speciesKey={int(species_key)}: {type(last_error).__name__}: {last_error}") from last_error
...
def freeze_country_plans(
    ...
    *,
    ...
    audit = {
        "schema_version": "global-availability-parity-country-freeze-v1",
        "status": "HISTORICAL_COUNTRY_PLANS_FROZEN_PRE_GEOMETRY_PRE_CANDIDATES_PRE_HELDOUT",
        "taxon_count": 48,
        "historical_facet_query_count": 48,
        "historical_facet_query_mode": "one focal query per already-frozen taxon; sequential with bounded retry",
        "ready_country_count": ready,
        "state_counts": {str(key): int(value) for key, value in state_counts.items()},
        "country_plan_canonical_sha256": _canonical_sha256(frame[["availability_pair_id", "speciesKey", "scientific_name", "country_plan_state", "selected_country_code", "historical_selected_country_count"]].to_dict(orient="records")),
