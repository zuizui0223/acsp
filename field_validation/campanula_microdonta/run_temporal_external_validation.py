#!/usr/bin/env python3
"""Temporal external validation of ACSP for Campanula microdonta.

Execution order is fixed: historical GBIF records through 2025 are retrieved,
the distance-excluded plant Top-5 is frozen, and only then are the independent
2026 field detections read. Field GPS coordinates never enter candidate
construction, scoring, parameter selection, or Top-k selection.
"""
...
SURVEY_BOUNDS = (
    min(bounds[0] for bounds in ISLAND_BOUNDS.values()),
    min(bounds[1] for bounds in ISLAND_BOUNDS.values()),
    max(bounds[2] for bounds in ISLAND_BOUNDS.values()),
    max(bounds[3] for bounds in ISLAND_BOUNDS.values()),
)
...
def fetch_historical_gbif(cap: int) -> tuple[pd.DataFrame, dict[str, Any]]:
    match = request_json(GBIF_MATCH_URL, {"name": SCIENTIFIC_NAME}, timeout=30)
    ...
    total = int(request_json(GBIF_SEARCH_URL, {**params, "limit": 0, "offset": 0}).get("count", 0))
    if total < 1:
        raise RuntimeError("GBIF returned no eligible historical coordinate records.")
    target = min(max(1, int(cap)), total)
    page_size = 300
    page_count = max(1, math.ceil(target / page_size))
    if total <= target:
        offsets = list(range(0, target, page_size))
        retrieval = "sequential"
    else:
        offsets = sorted(set(np.linspace(0, max(0, total - page_size), page_count, dtype=int).tolist()))
        retrieval = "representative_offsets"
    records: list[dict[str, Any]] = []
    for offset in offsets:
        limit = min(page_size, max(1, target - len(records)))
        payload = request_json(GBIF_SEARCH_URL, {**params, "limit": limit, "offset": int(offset)})
...
def island_center(island: str) -> tuple[float, float]:
    west, south, east, north = ISLAND_BOUNDS[island]
    return (south + north) / 2.0, (west + east) / 2.0


def assign_island(latitude: float, longitude: float) -> str:
    """Assign a candidate to one island, resolving overlapping rectangles by distance."""
    ...
    choices = containing or list(ISLAND_BOUNDS)
    return min(
        choices,
        key=lambda island: float(
            haversine_distance_m(
                lat,
                lon,
                np.array([island_center(island)[0]]),
                np.array([island_center(island)[1]]),
            )[0]
        ),
    )
...
def build_frozen_candidates(
    ...
    types = potential.get("candidate_type", pd.Series("", index=potential.index)).astype(str)
    potential = potential[
        ~types.str.contains("occurrence-supported|known-location|known anchor", case=False, na=False)
    ].reset_index(drop=True)
    scored = integrated_candidate_scores(potential, exclude_occurrence_derived=True)
    if scored.empty or "component_local_habitat_score" not in scored.columns:
        raise RuntimeError("No distance-excluded local-habitat candidate pool was produced.")
    ...
    scored["survey_area_id"] = [
        assign_island(lat, lon) for lat, lon in zip(scored["latitude"], scored["longitude"])
    ]
...
def island_constrained_recovery(
    ...
    for _, detection in clusters.iterrows():
        ...
        if available.empty:
            ...
            nearest = available.iloc[position]
            distance_km = float(distances[position] / 1000.0)
...
def island_random_benchmark(
    ...
    *,
    ...
    observed = recovery_summary(
        island_constrained_recovery(selected, clusters, radii_km), radii_km=radii_km
    ).set_index("radius_km")
    ...
    for iteration in range(1, max(1, int(iterations)) + 1):
        ...
        summary = recovery_summary(
            island_constrained_recovery(sampled, clusters, radii_km), radii_km=radii_km
        )
        for item in summary.itertuples(index=False):
            random_rows.append({
                "iteration": iteration,
                "radius_km": float(item.radius_km),
                "random_detection_recall": float(item.detection_recall),
            })
...
def main() -> None:
    ...
    field_rows, clusters = cluster_field_detections(
        locations, cluster_radius_m=float(args.cluster_radius_m), area_col="island"
    )
    ...
    regional_recovery = detection_recovery_table(
        selected, clusters, candidate_id_col="site_id", area_col="survey_area_id"
    )
    ...
    within_recovery = island_constrained_recovery(selected, clusters)
    ...
    within_recovery.to_csv(output / "within_island_detection_recovery.csv", index=False)
    within_summary.to_csv(output / "within_island_recovery_summary.csv", index=False)
    within_benchmark.to_csv(output / "within_island_same_pool_random_benchmark.csv", index=False)
    within_draws.to_csv(output / "within_island_same_pool_random_draws.csv", index=False)

    primary_regional = regional_benchmark[
        np.isclose(regional_benchmark["radius_km"], PRIMARY_RADIUS_KM)
    ].iloc[0]
    primary_within = within_benchmark[
        np.isclose(within_benchmark["radius_km"], PRIMARY_RADIUS_KM)
    ].iloc[0]
    result = {
        ...
        "candidate_island_allocation": {
            str(key): int(value) for key, value in pool["survey_area_id"].value_counts().sort_index().items()
        },
        "selected_candidates": int(len(selected)),
        "independent_detection_clusters": int(len(clusters)),
        "selected_island_allocation": {
            str(key): int(value) for key, value in selected["survey_area_id"].value_counts().sort_index().items()
        },
