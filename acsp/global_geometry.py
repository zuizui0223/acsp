...
This provider preserves the frozen research provider mechanics. It converts the confirmed GBIF
...
def fetch_geoboundaries_country_geometry(country_code: str) -> CountryLandGeometry:
    ...
    code = str(country_code).strip().upper()
    _, supported = _provider_inventory()
    if code not in supported:
        raise ValueError(f"country {code} is outside frozen provider coverage")
