#!/usr/bin/env python3
"""Predeclare the untouched factorial cohort for ACSP Practical Core confirmation.

This script only resolves species sampling frames and draws taxon-region pairs.
It never fetches focal occurrence rows, builds candidates, runs the Practical
Core, GRTS, fitted SDM, or calculates held-out recovery.
"""
...
PROTOCOL_PATH = Path("validation/acsp_practical_core_untouched_confirmation_protocol.json")
EXPECTED_PROTOCOL = "cd27dc5416057b574e58742bfe001307338a20aedb63ce1bac431588dc58eee9"
...
def _canonical_fingerprint(path: Path) -> tuple[dict[str, object], str]:
    ...
    if expected != calculated or calculated != EXPECTED_PROTOCOL:
        raise ValueError(
            "ACSP Practical Core confirmation protocol fingerprint mismatch: "
...
def run(protocol_path: Path, output: Path) -> dict[str, object]:
    ...
    result = {
        ...
        "practical_core_fingerprint": str(protocol["practical_core"]["protocol_fingerprint"]),
