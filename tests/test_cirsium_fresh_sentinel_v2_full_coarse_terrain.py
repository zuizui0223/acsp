...
import research.run_cirsium_fresh_sentinel_v2_full_coarse_terrain as mod
...
def _fake_attach(frame: pd.DataFrame):
    ...
    out["aspect_sin"] = 1.0
    out["aspect_cos"] = 0.0
    out["coarse_terrain_status"] = mod.COMPLETE
    out["coarse_terrain_provider_error_class"] = ""
    return out, {
...
def test_all_shards_assemble_to_exact_frozen_candidate_order(
    tmp_path: Path,
    monkeypatch,
) -> None:
    reference = _reference_frame()
    monkeypatch.setattr(mod, "frozen_outer_frame", lambda: reference.copy())
    monkeypatch.setattr(mod, "attach_coarse_terrain_primitives", _fake_attach)

    root = tmp_path / "shards"
    for shard_id in range(mod.SHARD_COUNT):
        manifest = mod.run_shard(shard_id, root / f"terrain-shard-{shard_id}")
    ...
    assert summary["stage3_source_execution_ready"] is True
