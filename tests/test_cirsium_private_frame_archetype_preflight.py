...
def test_archetype_preflight_reports_exact_private_source_blockers():
    table, summary = build_preflight(_requirements(), _cohort())
    assert list(table.cohort_unit_id) == ["CIR03", "CIR08", "CIR02"]
    assert summary["ready_units"] == 0
    assert summary["blocked_units"] == 3
    cir03 = table.set_index("cohort_unit_id").loc["CIR03"]
    assert cir03.required_private_inputs == "primary_anchor_geometry|gsi_dem_snapshot"
    cir08 = table.set_index("cohort_unit_id").loc["CIR08"]
    assert "gsi_coastline_snapshot" in cir08.required_private_inputs
    assert "target_ecological_component" in cir08.required_private_inputs
    cir02 = table.set_index("cohort_unit_id").loc["CIR02"]
    assert "sentinel_support_input" in cir02.required_private_inputs


def test_preflight_fails_closed_after_outcome_opening():
    with pytest.raises(ValueError, match="field outcome already opened"):
