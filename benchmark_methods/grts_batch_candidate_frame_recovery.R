...
# QA recovery adapter for the frozen Practical Core confirmation.
# This is the same official spsurvey::grts contract as the predeclared batch
# comparator, with the internal fold variable renamed because `repeat` is an R
# reserved word. It reads only the already-frozen pre-outcome candidate frame.
...
parse_args_recovery <- function(args) {
  out <- list(
    input = NULL,
    output = NULL,
    draws = 50L,
...
if (sys.nframe() == 0L) {
  args <- parse_args_recovery(commandArgs(trailingOnly = TRUE))
  if (is.null(args$input) || is.null(args$output)) stop("--input and --output are required")
  run_grts_batch_recovery(
    input = args$input,
    output = args$output,
    draws = args$draws,
